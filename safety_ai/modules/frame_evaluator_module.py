...
class FrameEvaluator():
    ...
    def evaluate_frame(self, frame_info:np.ndarray):
        ...
        evaluation_result = {
            ...
            "is_violation_detected": None, # If any violation is detected for the frame
        }   
        ...
        #"detections": [], # Contains multiple persons: List of dict of detection results for each person: {"bbox_class_name": str, "bbox_confidence": float, "nbbox": [x1, y1, x2, y2], "normalized_keypoints": {$keypoint_name: [xn, yn, confidence]}}
        #================================================================================================
        ...
        for active_rule in frame_info['active_rules']: #rule_uuid, camera_uuid, rule_type, evaluation_method, rule_department, rule_polygon, threshold_value, fol_threshold_value
            ...
            if active_rule['rule_type'] == "restricted_area_violation":
            ...
            # Hardhat violation 
                ...
                if active_rule['evaluation_method'] == "v1":
                    ...
                    raise Exception(f"Unknown evaluation method: {active_rule['evaluation_method']} for rule type: {active_rule['rule_type']}")
            # Check person
            elif active_rule['rule_type'] == "check_person":
                if active_rule['evaluation_method'] == "v1":
                    self.__check_person_isg_v1(evaluation_result=  evaluation_result, rule_info = active_rule)
                else:
                    raise Exception(f"Unknown evaluation method: {active_rule['evaluation_method']} for rule type: {active_rule['rule_type']}")
            else:
        ...
        for violation_person_nbbox in evaluation_result['violation_person_nbboxes']:
            ...
            if violation_person_nbbox['violation_type'] == "restricted_area_violation":
                ...
                picasso_module.draw_image_on_frame(frame=evaluation_result['processed_cv2_frame'], image_name="red_hardhat_transparent_with_background", x=bbox[2]+padding, y=bbox[1]-padding, width=icon_max_size, height=icon_max_size, maintain_aspect_ratio=True)
