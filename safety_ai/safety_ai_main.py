...
while True:
    ...
    if time.time() - last_time_violations_reported > 150:
    ...
    if len(evaluation_results) > 0: print(f"len(evaluation_results): {len(evaluation_results)}, remaining time: {150 - (time.time() - last_time_violations_reported):.2f} seconds")
