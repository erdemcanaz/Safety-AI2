    ...
    def apply_callbacks(self, redraw_items:bool = False, program_state:List=[1,0,0], callback_results:List=[], released_focus_identifiers:List=[]):
        ...
        for callback in callback_results:
            ...
            if callback == ["left_clicked_callback", self.previous_page_button.identifier, True]:
                ...
                result = self.api_dealer.create_camera_info(camera_ip_address=camera_ip_address, username=username, password=password, camera_region=camera_region if camera_region != "" else None, camera_status=camera_status)
                if result[0]:
                    self.popup_dealer.append_popup({"background_color":(0,255,0), "created_at":time.time(), "duration":3, "text":"Kamera eklendi"})
                else:
                    self.popup_dealer.append_popup({"background_color":(255,0,0), "created_at":time.time(), "duration":3, "text":result[2]["detail"]})
                self.reset_page_frame()
                return
                ...
                pprint.pprint(self.cameras[selected_index]) #[True, 200, {'camera_info': [{'date_created': '2024-08-26 08:44:56', 'date_updated': '2024-08-26 08:44:56', 'camera_uuid': '92051d28-3402-46cd-952b-d2ba2782443e', 'camera_ip_address': '1.1.1.1', 'camera_region': 'Henüz Atanmadı', 'camera_description': 'Henüz Atanmadı', 'NVR_ip_address': '', 'stream_path': '/asdaf', 'camera_status': 'active'}, ...]]
