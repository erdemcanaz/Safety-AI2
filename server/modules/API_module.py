...
import uuid, random, datetime, cv2, base64
...
class ViolationUUIDRequest(BaseModel):
    violation_uuid: str
...
async def get_violation_image_with_uuid(violation_uuid: ViolationUUIDRequest, current_user: User = Depends(get_current_user)):
    ...
    print(f"Violation UUID: {violation_uuid.violation_uuid}")
...
async def get_camera_configs_json(current_user: User = Depends(get_current_user)):
    ...
    with open(CAMERA_CONFIGS_PATH, "r") as f:
        CAMERA_CONFIGS_JSON: Dict[str, Dict[str, str]] = json.load(f)["cameras"]

    return {"list_": CAMERA_CONFIGS_JSON}

@app.post("/get_camera_frame", response_model=DictResponse)
async def get_camera_frame(camera_credentials: CameraCredentials, current_user: User = Depends(get_current_user)):
    if "KAMERALAR_APP" not in current_user.allowed_tos or "KURALLAR_APP" not in current_user.allowed_tos:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="User is not authorized for this app",
            headers={"WWW-Authenticate": "Bearer"},
    ...
    url = f'rtsp://{camera_credentials.username}:{camera_credentials.password}@{camera_credentials.camera_ip_address}/{"profile2/media.smp"}'
    ...
    if ret:
        ...
        image_base_64 = base64.b64encode(encoded_image).decode('utf-8')

    cap.release()
    dummy_dict = {
            "camera_ip_address": camera_credentials.camera_ip_address ,
