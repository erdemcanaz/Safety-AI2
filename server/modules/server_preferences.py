...
MINIMUM_USEFULNESS_SCORE_TO_CONSIDER = 0.2 # The minimum usefulness score that a camera can have. If the camera's usefulness score is less than this value, it is set to zero
MINIMUM_EVALUATION_PROBABILITY = 0.01 # The minimum probability that a camera will be evaluated. If the camera's calculated evaluation probability is less than this value, it is set to this value
GEOMETRIC_R = 0.25 # The evaluation probability of a camera is calculated as a geometric series. The first term is 1, and the common ratio is this value. The probability is calculated as 1 + 1*EVALUATION_PROBABILITY_GEOMETRIC_SERIES_MULTIPLIER + 1*EVALUATION_PROBABILITY_GEOMETRIC_SERIES_MULTIPLIER^2 + ...
