...
class ServerFailurePage():
    ...
    def do_page(self, program_state:List[int]=None, cv2_window_name:str = None,  ui_frame:np.ndarray = None, active_user:object = None, mouse_input:object = None):
        ...
        if mouse_input.get_last_leftclick_position() is not None:
            ...
            if self.__is_xy_in_bbox(x, y, self.CONSTANTS["retry_button"]): # -> direct to login page
                program_state[0] = 1
                program_state[1] = 0
                program_state[2] = 0
        ...
        # Draw UI
        picasso.draw_image_on_frame(ui_frame, image_name="server_failure_page_template", x=0, y=0, width=1920, height=1080, maintain_aspect_ratio=True)  
